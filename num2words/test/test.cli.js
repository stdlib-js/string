...
tape( 'the command-line interface converts a number to a word representation', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'13\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface converts a number to a word representation (lang=de)', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'193\';  process.argv[ 3 ] = \'--lang=de\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface supports use as a standard stream', opts, function test( t ) {
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			t.strictEqual( stdout.toString(), 'ten\nthirteen\n', 'expected value' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
	}
...
tape( 'the command-line interface supports use as a standard stream (lang=de)', opts, function test( t ) {
	var cmd = [
		'printf "10\n13"',
		'|',
		EXEC_PATH,
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			t.strictEqual( stdout.toString(), 'zehn\ndreizehn\n', 'expected value' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
	}
