...
tape( 'the command-line interface prints an input string repeated a specified number of times', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'--n=4\'; process.argv[ 3 ] = \'world\'; require( \''+fpath+'\' );"'
	];
	...
	function done( error, stdout, stderr ) {
		...
		if ( error ) {
			...
			str = stdout.toString();
			t.strictEqual( str === 'worldworldworldworld\n', true, 'prints expected value to `stdout`' );
