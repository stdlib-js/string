...
tape( 'the command-line interface prints the part of a string after last occurrence of a specified substring.', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'--search=b\'; process.argv[ 3 ] = \'boopbeep\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface prints the part of a string after last ocurrence of a specified substring (custom start index).', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
	...
	function done( error, stdout, stderr ) {
		...
		if ( error ) {
			...
			str = stdout.toString();
			t.strictEqual( str === 'oopbeep\n', true, 'prints `oopbeep` string to `stdout`' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
	}
...
tape( 'the command-line interface supports use as a standard stream (custom start index)', opts, function test( t ) {
	var cmd = [
		'printf "beep boop\nboop beep"',
		'|',
		EXEC_PATH,
