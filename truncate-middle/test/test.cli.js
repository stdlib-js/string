...
tape( 'the command-line interface truncates a string in the middle', opts, function test( t ) {
	var opts;
	var cmd;
...
tape( 'the command-line interface truncates a string in the middle (custom sequence)', opts, function test( t ) {
	var opts;
	var cmd;
	...
	cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'beep boop\'; process.argv[ 3 ] = \'--len=5\'; process.argv[ 4 ] = \'--seq=!\'; require( \''+fpath+'\' );"'
	];
	opts = {};
...
tape( 'the command-line interface supports use as a standard stream (custom sequence)', opts, function test( t ) {
	var cmd = [
		'printf "beep boop\nHello World!"',
		'|',
		EXEC_PATH,
