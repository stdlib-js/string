...
tape( 'the command-line interface prints the part of a string after a specified substring.', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'--search=boop\'; process.argv[ 3 ] = \'boopbeep\'; require( \''+fpath+'\' );"'
	];
...
tape( 'the command-line interface prints the part of a string after a specified substring (custom start index).', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		'-e',
	...
	function done( error, stdout, stderr ) {
		...
		if ( error ) {
			...
			str = stdout.toString();
			t.strictEqual( str === '\n', true, 'prints empty string to `stdout`' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
	}
...
tape( 'the command-line interface supports use as a standard stream', opts, function test( t ) {
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			t.strictEqual( stdout.toString(), 'boop\nbeep\n', 'expected value' );
			t.strictEqual( stderr.toString(), '', 'does not print to `stderr`' );
		}
		t.end();
	}
